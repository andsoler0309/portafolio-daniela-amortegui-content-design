...
import { workProjects, workProjectsEn } from "@/lib/data";
import type { WorkProject, ContentBlock } from "@/lib/data";
...
/* ─── SECTION IDs ───
   El orden es fijo (Desafío → Mi rol → Enfoque → Solución → Impacto) y cada
   caso muestra solo las secciones que tenga definidas en los datos. */
function useSections(project: WorkProject) {
  const { t } = useI18n();
  const all = [
    { id: "desafio", label: t("work.sectionNav.desafio"), present: true },
    { id: "rol", label: t("work.sectionNav.rol"), present: true },
    { id: "enfoque", label: t("work.sectionNav.enfoque"), present: !!project.enfoque },
    { id: "solucion", label: t("work.sectionNav.solucion"), present: !!project.solucion },
    { id: "impacto", label: t("work.sectionNav.impacto"), present: true },
  ];
...
/* ─── ROL ─── */
function RolSection({ project }: { project: WorkProject }) {
...
function DesafioSection({ project }: { project: WorkProject }) {
  const { t } = useI18n();
  return (
...
/* ─── SOLUCIÓN ─── */
function SolucionSection({ project }: { project: WorkProject }) {
...
function EnfoqueSection({ project }: { project: WorkProject }) {
...
function ImpactoSection({ project }: { project: WorkProject }) {
...
function ProjectHero({ project }: { project: WorkProject }) {
  const { t } = useI18n();
  return (
    <div className="bg-bg-primary" style={{ paddingTop: "clamp(4rem, 8vw, 7rem)", paddingBottom: "clamp(3.5rem, 6vw, 5rem)" }}>
      <div className="container-main">
        ...
        {/* Subtitle — one-line framing of the case */}
        {project.subtitle && (
          <motion.p
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
...
/* ─── SINGLE PROJECT VIEW ─── */
function ProjectView({ project }: { project: WorkProject }) {
  const { t } = useI18n();
  ...
  return (
    ...
    >
      ...
      {/* Sections — Desafío → Mi rol → Enfoque → Solución → Impacto */}
      <div className="container-main">
        <section id={`${project.id}-desafio`} className="scroll-mt-52" style={{ padding: "5rem 0 5rem" }}>
          <SectionLabel label={t("work.sectionLabel.desafio")} />
          <br/>
        ...
        <section id={`${project.id}-rol`} className="scroll-mt-52" style={{ padding: "5rem 0 5rem" }}>
          <SectionLabel label={t("work.sectionLabel.rol")} />
          <br/>
        ...
        {project.enfoque && (
          <>
            ...
            <section id={`${project.id}-enfoque`} className="scroll-mt-52" style={{ padding: "5rem 0 5rem" }}>
              <SectionLabel label={t("work.sectionLabel.enfoque")} />
              <br/>
        ...
        {project.solucion && (
          <>
            ...
            <section id={`${project.id}-solucion`} className="scroll-mt-52" style={{ padding: "5rem 0 5rem" }}>
              <SectionLabel label={t("work.sectionLabel.solucion")} />
              <br/>
        ...
        <section id={`${project.id}-impacto`} className="scroll-mt-52" style={{ padding: "5rem 0 6rem" }}>
          <SectionLabel label={t("work.sectionLabel.impacto")} />
          <br/>
...
export function WorkPageClient() {
  ...
  const projects = locale === "en" ? workProjectsEn : workProjects;
          ...
          <div className="flex items-center overflow-x-auto scrollbar-hide flex-1" style={{ gap: 0 }}>
            {projects.map((project, i) => (
              <button
                ...
                style={{ height: "68px", padding: "0 1.5rem", borderRight: "1px solid rgba(212,197,176,0.25)" }}
